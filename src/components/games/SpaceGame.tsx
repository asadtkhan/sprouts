...
export function SpaceGame({ stage, health }: Props) {
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={`Rocket companion, stage ${s} of ${MAX}. Tap to fire the boosters.`}
      onClick={react}
      onKeyDown={(e) => {
    ...
    >
      ...
      <svg viewBox="0 0 320 320" className="w-full h-full">
        ...
        <g>
          ...
          {capped >= 8 && (
            <g>
              ...
              <text x="240" y="76" textAnchor="middle" fontSize="18">
                💥
              </text>
            </g>
          )}
        ...
        {/* rocket bobbing (wobbles when critical); nested boost group plays on tap */}
        <g
          key={`poke-${poke}`}
          style={poke ? { animation: "rocketBoost 0.55s ease-out" } : undefined}
        >
          <g
            style={{
              animation: `${critical ? "rocketWobble" : "rocketBob"} 2.2s ease-in-out infinite`,
            }}
          >
            <g filter="url(#space-soft)" transform={`translate(${100 + capped * 12}, ${rocketY})`}>
              ...
              {takingOff && (
                <g>
                  <circle
                    ...
                    r="8"
                    fill="#dcdce6"
                    style={{ animation: "smokePuff 1.4s ease-out infinite" }}
                  />
                  <circle
                    cx="6"
                    cy="72"
                    r="6"
                    fill="#e8e8f0"
                    style={{ animation: "smokePuff 1.4s ease-out 0.5s infinite" }}
                  />
                  <circle
                    cx="18"
                    cy="70"
                    r="7"
                    fill="#d0d0dc"
                    style={{ animation: "smokePuff 1.4s ease-out 0.9s infinite" }}
                  />
                </g>
              )}
              {(capped > 0 || takingOff) && (
                ...
                >
                  <path
                    d="M0 38 Q-10 58 0 78 Q10 58 0 38 Z"
                    fill={lowFuel ? "#ffb08a" : "#ff8a3a"}
                    opacity="0.55"
                  />
              ...
              <path
                d="M-12 -30 Q0 -50 12 -30 L12 30 L-12 30 Z"
                fill="url(#space-sheen)"
                opacity="0.5"
              />
